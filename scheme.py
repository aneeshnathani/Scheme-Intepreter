...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return proc.eval_call(expr.second,env)
...
def eval_all(expressions, env):
    ...
    while expressions.second:
        evaluateindividually = scheme_eval(expressions.first,env)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Number of argument values and formal parameters differ')
        while formals:
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        arguments = operands.map(lambda singleoperand: scheme_eval(singleoperand, env))
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        arguments = scheme_apply(self, operands, env)
        thunk = Thunk(complete_eval(arguments), env)
        return complete_eval(thunk)
        # END Problem 21
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    letenv = env.make_child_frame(nil, nil)
    while bindings:
      check_form(bindings.first,0,2)
      check_formals(Pair(bindings.first.first,nil))
      letenv.define(bindings.first.first, scheme_eval(bindings.first.second.first,env))
